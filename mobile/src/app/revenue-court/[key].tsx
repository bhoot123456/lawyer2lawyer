...
import { getRevenueCourtPhase8 } from "@/services/revenueCourtApi";
...
export default function RevenueCourtTopicScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const res = await getRevenueCourtPhase8();
        setData(res);
      } finally {
        setLoading(false);
      }
    ...
    <ScrollView contentContainerStyle={styles.container}>
      <GlassCard borderColor={colors.border.gold} accent={colors.accent.gold}>
        <View style={styles.headerRow}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.subtitle}>Procedure • Documents • Fees • Timeline • Downloads • FAQs</Text>
          </View>
        </View>
      </GlassCard>
      ...
      {!loading && !topic ? (
        <View style={{ marginTop: 16 }}>
          ...
          <Pressable
            onPress={() => router.push("/revenue-court")}
            ...
            {downloads.length ? (
              <View style={styles.downloadList}>
                {downloads.map((d, idx) => (
                  <Pressable
                    key={`${d.url}-${idx}`}
                    style={({ pressed }) => [styles.downloadRow, pressed ? { opacity: 0.92 } : null]}
                    onPress={() => Linking.openURL(d.url)}
                  >
                    <Ionicons name="download-outline" size={16} color={colors.accent.gold} />
