...
import { getJudgesByDistrict } from "@/services/judgeDirectoryApi";
    ...
    <View style={styles.headerContainer}>
      ...
      <View style={styles.headerContent}>
        <View style={styles.headerIconWrap}>
          <Ionicons name="people-outline" size={22} color={ACCENT_DARK} />
        </View>
        <View>
          <Text style={styles.headerTitle}>
            {districtMeta.district || "Judges List"}
          </Text>
...
const ErrorState = memo(function ErrorState({
  ...
  return (
    <View style={styles.centerContainer}>
      ...
      <Text style={styles.errorTitle}>Unable to Load Judges</Text>
...
const EmptyState = memo(function EmptyState() {
  return (
    <View style={styles.centerContainer}>
      ...
      <Text style={styles.emptyTitle}>No Judges Found</Text>
      <Text style={styles.emptyMessage}>
        No judges are currently listed for this district. Please check back
        later.
      </Text>
    </View>
  );
  ...
  return (
    <View style={styles.introBlock}>
      ...
      <Text style={styles.introText}>
        {districtMeta.total} Judge{districtMeta.total !== 1 ? "s" : ""} Available
      </Text>
    </View>
  );
});
...
export default function JudgesListScreen() {
  const { complexId, districtId } = useLocalSearchParams<{
    complexId: string;
    districtId: string;
  }>();

  const [judges, setJudges] = useState<DistrictCourtJudge[]>([]);
  ...
  const fetchJudges = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        ...
        const response = await getJudgesByDistrict(complexId, districtId);
  ...
  const renderJudgeItem = useCallback(
    ({ item }: { item: DistrictCourtJudge }) => (
      <JudgeCard judge={item} />
    ),
    [],
  );

  const keyExtractor = useCallback(
    (item: DistrictCourtJudge) => item._id,
    [],
  );
  ...
  const headerComponent = useMemo(
    () => (
      <>
        ...
        {listHeader}
        <Text style={styles.sectionLabel}>Judge Directory</Text>
      </>
    ),
