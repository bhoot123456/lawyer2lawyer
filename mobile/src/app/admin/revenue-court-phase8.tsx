...
import {
  getAdminRevenueCourtPhase8,
  deleteAdminRevenueCourtPhase8,
  createAdminRevenueCourtPhase8,
  updateAdminRevenueCourtPhase8,
...
export default function AdminRevenueCourtPhase8Screen() {
  ...
  const fetchItems = useCallback(
    async (pg = 1) => {
      try {
        ...
        const res = await getAdminRevenueCourtPhase8(params);
        ...
        console.error("Fetch revenue court phase8 error:", err);
      } finally {
        setLoading(false);
        setRefreshing(false);
      }
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              await deleteAdminRevenueCourtPhase8(id);
  ...
  const handleSave = async (id: string) => {
    ...
    setSaving(true);
    try {
      const res = await updateAdminRevenueCourtPhase8(id, editForm);
      if (res?.success) {
        setItems((prev) =>
  ...
  const handlePublish = async (id: string) => {
    setSaving(true);
    try {
      const res = await updateAdminRevenueCourtPhase8(id, { status: "published" });
      if (res?.success) {
        setItems((prev) =>
          prev.map((x) => (x._id === id ? { ...x, ...res.data } : x)),
        );
  ...
  const handleUnpublish = async (id: string) => {
    setSaving(true);
    try {
      const res = await updateAdminRevenueCourtPhase8(id, { status: "draft" });
      if (res?.success) {
        setItems((prev) =>
          prev.map((x) => (x._id === id ? { ...x, ...res.data } : x)),
        );
  ...
  const handleCreate = async () => {
    ...
    try {
      await createAdminRevenueCourtPhase8({
        title: newTitle,
        key: newKey,
        topic: newTopic,
        content: newContent,
